# Introduction To Dynamic Web Content
    # Getting Data From The Server Request/Response Cycle
''' 
        Each time the user clicks on an anchor tag with an href = value to switch to a new page, the browser makes a connection to the web server and issues a "GET" request - to GET the content of the page at the specified URL.
        The server returns the HTML document to the browser, which formats and displays the document to the user.
'''   

# HyperTest Transfer Protocol (HTTP)
    # Uniform Resource Locator
'''
        Protocol    - http://
        Host        - www.google.com
        Document    - /page1
'''
    # HTTP
'''
        Makes a connection
        Request a document
        Retrieve the document
        Close the connection
'''
    # Request
'''
        A request message from a client to a server includes, within the first line of that message the method to be applied to the resource, the identifer of the resource, andt heprotocol verison in use.
'''

# Network Socket
    # TCP Connections / Sockets
'''

'''
